...
class Difficulty{
  ...
  private float increasePercentage = 1+10/100;
  private float decreasePercentage = 1-10/100;
  
  Difficulty(){  //Initialization
    this.ballColor = color(rgb[0],rgb[1],rgb[2]);
  ...
  void Increase(){  // Increase Difficulty level
    setBallColor( changeRGBbyPercentage(increasePercentage) );  //Lighten ballColor by 10%
    this.ballAcceleration *= increasePercentage; //Increase ballAcceleration 10%
  }
  void Decrease(){  // Decrease Difficulty level
    setBallColor( changeRGBbyPercentage(decreasePercentage) ); //Darken ballColor by 10%
    this.ballAcceleration *= decreasePercentage; //Decrease ballAcceleration 10%
  }
  
  private int[] changeRGBbyPercentage(float perc){  // Change the rgb color set by a percentage and return the new rgb color set.
    for (int i=0; i<this.rgb.length; i++){
      this.rgb[i] *= perc;
    }
    return this.rgb;  // Returning int[3]m as is the rgb.length
  }
  
  void setBallColor(int[] bc){  // Update the ballColor. Accepting int[3]
    this.ballColor = color(bc[0],bc[1],bc[2]);
  }
}
